...
The orchestration CLIs (`memory`, `workspace`, `tradeoffs`) are on `PATH` — call them as bare commands. They live in the `agents` dotfiles topic's `skills/bin/` and run from there, not from `~/Code`.
