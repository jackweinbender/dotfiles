...
2. **Merge the plan branch into the target** from that worktree — refs are shared, so no push of the plan branch is needed: `git -C <target-worktree> merge --no-ff <branch>`. On a conflict, run the `merge-conflicts` skill's Phase 0 diagnosis: a **squash-merge artifact** resolves mechanically with no hunks and the merge proceeds; **genuine textual overlap** between two plans is a **BLOCK** (the DAG order or scope is wrong) — report it, don't hand-resolve.
