...
4. **Merge-gate: a dependent plan executes only once its prerequisites are `DONE` and present on its `Target` branch.** We never auto-stack on unmerged branches.
   *Why:* every executor branches from a clean, real base (the tip of its Target). For an independent plan (Target = the default branch) the gate is the human merging the prerequisite's draft PR to base — the integration checkpoint stays the user's ("merging the PR is the user's decision"); for plans sharing an integration branch the gate is the prerequisite already merged into it. Either way small, reviewable PRs fall out for free. *If reversed (auto-chaining unmerged branches):* you get a tall stack of unreviewed branches, and one bad early plan poisons everything stacked on it.
...
7. **One plan = one branch = one worktree. Parallelism is at the plan level.** No `agentid` path segment; no fan-in merge of multiple agent worktrees.
   *Why:* the plan is the unit of branching, and git physically refuses to check out one branch in two worktrees. Plans that should run in parallel are made independent (no shared scope, no dep) by the planner, so their `<repo>-<branch>` worktrees already coexist without collision. "Merge several worktrees together onto one branch" reintroduces the cross-slice conflict resolution that plan-per-branch is designed to avoid. *If reversed:* coordination/merge complexity creeps back inside a unit that was supposed to be atomic. (An *integration branch* — plans sharing a non-default `Target` — does not break this: each plan still has its own branch/worktree and is reviewed independently; on approval its branch is merged into the shared Target **one at a time**, never combining live worktrees. The serialization keeps the atomic unit intact.)

8. **Read-only on source everywhere except the executor's worktree. The reviewer lands approved work, but never commits to or merges into the protected base.**
   *Why:* planners advise; the executor edits only its disposable worktree. After approval the reviewer *lands* the work — pushes the branch and opens a **draft** PR (independent plan), or merges into the plan's non-protected integration branch and pushes (integration plan) — so the work becomes reviewable (and reachable by review bots) without the human losing the integration decision. What stays inviolable: the canonical clone is never mutated, and the **protected base** (the repo default) is never committed to, merged into, or PR-merged by the skill — that is the human's call. *If reversed (skill merges PRs to base):* the advisor becomes an unreviewed integrator and the "you decide what merges" guarantee evaporates.
...
12. **TDD is opt-in per plan, and its concerns split across the seams.** A plan's `## Test plan` carries a `TDD: yes/no` seam marker; when yes, the planner enumerates the *behaviors to test* and the executor drives them red→green→refactor. The behavior-selection gate lives in **vetting/planning** (`vet`/`plan`, where the user is in the loop — `vet` pins the behaviors, `brief` carries the intent into `plan`); the red-green loop lives in the **executor** (the `tdd` skill); the test-quality audit lives in **review** (the Standards axis).
    *Why:* the executor is a cheaper, non-interactive subagent that must not improvise — so the interactive "which behaviors matter?" gate can't live there; it has to be resolved in the plan. Opt-in (not default-on) keeps config/docs/refactor/infra plans free of test-first ceremony that buys nothing. *If reversed:* either the executor invents a behavior list it was never given (and tests imagined behavior), or every plan pays TDD overhead whether or not there's new behavior to specify.

13. **`execute`'s review is two-axis (Spec | Standards), always fanned out to two parallel read-only sub-agents, aggregated-not-merged.** Spec judges the diff against the plan (done criteria, intent, scope); Standards judges conventions, code health, and test quality. Findings are reported under separate headings; APPROVE requires both clean. The reviewers find but never fix, and the done-criteria re-run + scope diff stay gates the execute session owns.
    *Why:* a diff that follows every convention while implementing the wrong feature must fail Spec even though Standards is clean, and vice-versa — bundling the axes lets one clean axis mask the other's failure. Two independent agents keep the axes from contaminating each other. *If reversed (one bundled pass):* "looks like the codebase, ships green" quietly approves the wrong behavior, which is exactly the failure the split prevents.
