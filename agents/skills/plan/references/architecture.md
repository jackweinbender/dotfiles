...
12. **TDD is opt-in per plan, and its concerns split across the seams.** A plan's `## Test plan` carries a `TDD: yes/no` seam marker; when yes, the planner enumerates the *behaviors to test* and the executor drives them red→green→refactor. The behavior-selection gate lives in **planning** (`plan`/`grillme`, where the user is in the loop); the red-green loop lives in the **executor** (the `tdd` skill); the test-quality audit lives in **review** (the Standards axis).
